...
*    with itself.
*/
__global__ void internal_level_build_naive(int parent_level_size, uint8_t *parents_level, int children_level_size, uint8_t *children_level) {
...
MerkleTreeGPU* build_merkle_tree_naive(size_t n_blocks, uint8_t* host_data_bytes, uint64_t* out_elapsed){
    ...
    while(children_level_size > 1){
        ...
        internal_level_build_naive<<<blocks_per_grid, THREADS_PER_BLOCK>>>(parent_level_size, dev_curr_parent_level, children_level_size, dev_curr_children_level);
        cudaDeviceSynchronize();
