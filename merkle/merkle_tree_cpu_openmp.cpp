...
*/
MerkleTreeCPU* host_build_merkle_tree_parallel(size_t n_blocks, uint8_t* host_data_bytes, bool sha256_windowed, uint64_t* out_elapsed, int* out_num_threads = nullptr) {
    ...
    #pragma omp parallel
    {
        if (omp_get_thread_num() == 0 && out_num_threads)
            *out_num_threads = omp_get_num_threads();
 ...
 */
uint8_t* get_host_hashed_proofs_parallel(ProofBatch* proof_batch, size_t n_proofs, bool sha256_windowed){
 ...
 */
bool* host_compute_merkle_proofs_parallel(ProofBatch* proof_batch, MerkleTreeCPU* merkle_tree, bool sha256_windowed, uint64_t* out_elapsed, int* out_num_threads = nullptr){
    
    size_t n_proofs = proof_batch->n_proofs;
    // merkle proof hashing
    uint8_t* hashed_proofs = get_host_hashed_proofs_parallel(proof_batch, n_proofs, sha256_windowed);
    // allocating results
